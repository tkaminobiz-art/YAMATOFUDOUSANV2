...
const y = await page.evaluate(() => {
  const h = [...document.querySelectorAll('h2')].find(x => x.textContent && x.textContent.includes('特別安い'));
  return h ? h.getBoundingClientRect().top + window.scrollY - 80 : 0;
});
await page.evaluate(py => window.scrollTo(0, py), y);
await page.waitForTimeout(600);
await page.screenshot({ path: '/tmp/yamato-critique-snap/mech-newheading.png' });
console.log('✓ y=' + y);
...
await page.screenshot({ path: '/tmp/yamato-critique-snap/mech-newheading-mob.png' });
