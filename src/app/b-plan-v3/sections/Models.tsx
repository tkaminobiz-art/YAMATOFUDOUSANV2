...
/**
 * Models — 新8「モデル」｜花→風→京（カタログの潔さ）。現 S08 を新構成へ移設。
 *
 * 確定フレームワーク 新14構成 新8「モデル」ビルドカード:
 * - 役割/段: ①大手と同品質の素材・装備。surface=ivory（白・カタログ）。主タイプ=T2/T5。
 *   id="product" を付与し V3Header/sitemap の `/#product` アンカーを修復する。
 * - 主役: t-h2（モデル選びの見出し）＋3 cover card（t-burn-sub の静止価格）。
 ...
 *   価格は BRAND-TRUTH §2.1 canonical 厳守。価値3円図は本節前には置かない（S06 末に配置済み）。
 * - CTA: 各カードに静かな tertiary（text-link → /money）。
 *
 * 契約: `export default function Models(): JSX.Element`（props 無し・サーバー既定）。
 * モーション（useScrollIn）が要る図だけを子コンポーネント（S08.client.tsx）に切り出している。
 */
export default function Models() {
  return (
    <SectionShell id="product" surface="ivory" aria-label="商品ライン｜花・風・京">
      <div className="max-w-[760px]">
        <Eyebrow>Product Line</Eyebrow>

        <h2 className="t-h2 text-ink">
          広さも、予算も、あきらめ方ではなく選び方で決める。
        </h2>
