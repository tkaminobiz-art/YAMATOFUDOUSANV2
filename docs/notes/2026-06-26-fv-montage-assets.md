...
## 2026-06-29 追記｜1080p 再エンコード（神野さん指示「PC/スマホとも 1080p に」）

初版 720p は FV をフルブリード全幅表示するため PC で拡大ボケ＋低ビットレートでクロスフェードがモヤついていた。
**承認済みカット順を維持したまま** 高解像度マスターから 1920×1080 で再構築。

- カット順の確定方法: 初版 720p 動画から 30 フレームを 0.5s 刻みで抽出し、知覚ハッシュで許可リスト素材へ機械照合（最良一致 RMSE≈0.000〜0.003 の一意一致）。結果は本書「30枚モンタージュ構成」と完全一致。
- 確定マッピング（30フレーム）とビルド手順は **再現可能スクリプト [`scripts/build-fv-montage.mjs`](../../scripts/build-fv-montage.mjs)** に集約（初版はビルド手順未保存だった）。`node scripts/build-fv-montage.mjs` で再生成可。
- 本物の 1080p になるのは外観（マスター 2752×1536・12フレーム）＋ newsozai 内観（2400×1600・4フレーム）の計16枚。works-parts 系（原本 918px・14フレーム）は原本がそれ以上に無いため 1080p へ拡大（ただし高ビットレートで初版より改善・暗オーバーレイ下の 0.5s カットのため粗は目立たない）。
- 出し分けなし（PC/スマホ共通 1080p）。reduced-motion / Save-Data は従来どおり poster 静止のみ（`S01Hero.client.tsx`・コード変更なし）。

| ファイル | 旧(720p) | 新(1080p) | 仕様 |
|---|---|---|---|
| `yamato-fv-montage.webm` | 1.41MB | **3.87MB** | VP9 2パス・1920×1080・~1.67Mbps |
| `yamato-fv-montage.mp4` | 1.79MB | **5.00MB** | H.264 2パス・~2.2Mbps（Safari/iOS 経路・肥大回避でCRF任せにせず2パス固定） |
| `yamato-fv-montage-poster.webp` | 0.09MB | **0.19MB** | 1920×1080（三山木 夕暮れマスター・ImageMagick生成） |

※体感速度は据え置き: poster(0.19MB)が即表示され動画は `preload="metadata"` で背後ストリーミング。総尺は 18.5s（初版 18.03s 相当）。
※旧 720p 一式は作業時バックアップ済（必要なら復元可）。
