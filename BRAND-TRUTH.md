...
### 4.4b 運搬費・工事中の駐車場代 ─「別途請求しない／総額に含む」（2026-06-29 神野さん確認）

やまとは、工事に必要な資材の運搬や、工事中の駐車スペースにかかる費用を **別途請求しない**（総額に含めて提示する）。他社では見積もり後に別途加算されやすい実費を、やまとは後出ししない。

**How to apply:**
- 言い回しは「別途いただきません／総額に含みます」を主軸に（やまとの請求方針＝billing practice の事実摘示。費用の物理的存在を否定する「一切ない」「ゼロ」は使わない）
- 地盤改良費（§4.4）・つなぎ融資（§4.3）と同じ「あとから出やすい不安を先に消す」クラスタで扱う
- 価格訴求でなく「総額の見通しが崩れにくい」へ接続（§4.4 と同じ意味付け）
- ❌「追加費用ゼロ」「一切かからない」（過剰断定・§4.6） → ✅「別途請求しません／総額に含みます」
